...
func TestInsertWithNullableField(t *testing.T) {
	...
	arr, _ := data[0][arrayFieldName].(*schemapb.ScalarField)
	assert.Equal(t, int64(9999999999999999), arr.GetLongData().GetData()[0])
	log.Info("lxg", zap.Any("data", data))
	...
	log.Info("tt", zap.Any("fieldData", fieldData))
}
...
func TestBuildQueryRespsWithNullableField(t *testing.T) {
	outputFields := []string{"XXX", "YYY"}
	...
	for _, c := range cases {
		rows, err := buildQueryResp(int64(0), outputFields, newNullableFieldData([]*schemapb.FieldData{}, c.dataType), generateIDs(schemapb.DataType_Int64, 3), DefaultScores, true)
		assert.Equal(t, nil, err)
		assert.Equal(t, 3, len(rows))
		actualData := rows[1][c.name]
		assert.Equal(t, nil, actualData)
	}
}
