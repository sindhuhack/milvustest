...
func (t *clusteringCompactionTask) getScalarAnalyzeResult(ctx context.Context) error {
	...
	buckets, containsNull := t.splitCluster(analyzeDict)
	...
	for _, paths := range fieldBinlogPaths {
		...
		for {
			...
			if (remained+1)%100 == 0 {
				...
				bufferFull := clusterBuffer.writer.IsFull()
				if clusterBuffer.currentSegmentRowNum.Load() > t.plan.GetMaxSegmentRows() || bufferFull {
...
func (t *clusteringCompactionTask) splitCluster(dict map[interface{}]int64) ([][]interface{}, bool) {
